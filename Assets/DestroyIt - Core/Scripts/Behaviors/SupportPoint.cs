...
{
    /// <summary>
    /// This script manages a single support point (Trigger Collider) on an object. 
        ...
        void Start()
        {
            ...
            if (canSupport)
            {
                ...
                for(int i=0; i<hitInfo.Length; i++)
                {
                    ...
                    // add a joint cleanup script if there isn't already one attached.
                    if (transform.parent.GetComponent<Collider>().attachedRigidbody.gameObject.GetComponent<JointCleanup>() == null)
                        transform.parent.GetComponent<Collider>().attachedRigidbody.gameObject.AddComponent<JointCleanup>();
